...
- Automated browser assertions passed at 1440x1000, 1024x768, and 390x844: no horizontal overflow; configuration does not overlap Chat; composer/Send are reachable; multi-Session names and connection state remain inside the tablet sidebar; the mobile New session button receives pointer input; Session actions stay in the first modal viewport.
- Keyboard and state assertions passed: focus starts inside Session setup, Tab/Shift+Tab remain trapped, Escape closes and restores the opener, the application background is inert/`aria-hidden`, and mode/permission controls report `aria-pressed`. A rejected `..` Session id keeps every draft field and shows a localized inline error; a subsequent valid Echo Session succeeds without stale Connection errors.
