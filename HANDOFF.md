...
- [Windows Packages run 29484858873](https://github.com/MattSureham/quorum/actions/runs/29484858873) is the current packaged acceptance baseline on `548b67d`. It passed all 166 tests, parsed real PDF and DOCX files through the compiled Windows sidecar smoke, built the Web UI and unsigned NSIS installer, validated the portable layout, and uploaded the portable, sidecar, bundle-output, and NSIS artifact groups. A tester can download the portable artifact directly; cloning or pulling the repository is not required to run it.
