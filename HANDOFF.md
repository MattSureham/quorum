...
- The daemon validates MIME/data-URL agreement, decoded byte size, and PDF/DOCX file signatures before parsing. DOCX central-directory metadata is preflighted for required OOXML parts, entry count, per-entry expanded size, and total expanded size before decompression. `unpdf` extracts embedded PDF text and Mammoth extracts DOCX paragraph text. Parser failure is visible on the document card; a PDF with no embedded text is explicitly marked as requiring OCR rather than treated as understood. OCR and legacy `.doc` are not implemented.
