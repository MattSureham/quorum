...
- Windows follow-up runs [29311397456](https://github.com/MattSureham/quorum/actions/runs/29311397456) and [29311586815](https://github.com/MattSureham/quorum/actions/runs/29311586815) passed typecheck but timed out in Git/fs.watch integration tests. The shared-lock test now pre-initializes its repository so it isolates coordinator/lease behavior, waits for both editing adapters directly, and still asserts peak concurrency is one. The out-of-band watcher test retains its behavior assertion with a Windows-specific file-notification window. A fresh run is required.
