...
- **P0 shared workspace write lock: implementation and deadline semantics corrected; fresh Windows CI pending.** Sessions using the same canonical `realpath` share one workspace coordinator, write mutex/checkpoint queue, and watcher. Lease queue time no longer consumes the agent execution deadline. Conflicting active branches are rejected.
...
- Windows follow-up run [29311397456](https://github.com/MattSureham/quorum/actions/runs/29311397456) passed typecheck but still failed the shared-workspace test while it waited for two WebSocket `turn_completed` observations. The test now waits for both editing adapters to finish directly and still asserts peak concurrency is one, separating workspace-lock correctness from WebSocket event-delivery timing. A fresh run is required. Earlier run [29304261126](https://github.com/MattSureham/quorum/actions/runs/29304261126) failed at the same assertion with `92/93` tests passing.
...
- Local verification after the follow-up: `pnpm typecheck` and `pnpm test` (`98/98`) pass. The earlier Web/smoke/Tauri verification remains valid for the preceding changes; rerun the full matrix and Windows workflow for final acceptance.
