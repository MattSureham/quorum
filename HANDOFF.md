...
- [Windows Packages run 29482869976](https://github.com/MattSureham/quorum/actions/runs/29482869976) is the current packaged acceptance baseline on `a91cfe7`. It passed all 163 tests, parsed real PDF and DOCX files through the compiled Windows sidecar smoke, built the Web UI and unsigned NSIS installer, validated the portable layout, and uploaded the portable, sidecar, bundle-output, and NSIS artifact groups. A tester can download the portable artifact directly; cloning or pulling the repository is not required to run it.
...
- The earlier `fabc213` remediation checkpoint was green: `pnpm typecheck`; `pnpm test` with **150/150** tests across 24 files; Web production build; EventLog, shared-session, source-sidecar, Node fallback, and Bun compiled sidecar smokes; and `pnpm desktop:check` including Rust `cargo check`. The superseding 163-test current result is recorded in final browser acceptance below.
...
- The in-app browser had no available instance after the required connection diagnostics, so final interaction and screenshot acceptance used an isolated temporary sidecar/database/workspace plus standalone Playwright driving local headless Chrome. No private credential database or repository workspace was used; the temporary service was stopped afterward.
- Automated browser assertions passed at 1440x1000, 1121x768, 1024x768, 901x768, and 390x844: no horizontal overflow; configuration does not overlap Chat; policy controls and Interrupt remain visible; composer/Send are reachable; multi-Session names and connection state remain inside the tablet sidebar; the mobile New session button receives pointer input; Session actions stay in the first modal viewport. The 901 px Chat feed measures about 403 px instead of the reported 44 px.
- Keyboard and state assertions passed: focus starts inside Session setup, Tab/Shift+Tab remain trapped, Escape closes and restores the opener, the application background is inert/`aria-hidden`, and mode/permission controls report `aria-pressed`. A rejected `..` Session id keeps every draft field and shows a localized inline error; a subsequent valid Echo Session succeeds without stale Connection errors. A delayed `session_created` plus an injected unrelated WebSocket error confirms the pending form remains frozen and only the correlated success closes/switches it.
- Runtime assertions passed: one Echo participant returns exactly one reply per prompt; a second prompt posted during `settling` gets its own response in FIFO order without a third prompt; a 700 ms delayed FileReader disables Send and cannot separate text from its image; deleting loaded `A.png` while `B.png` is pending leaves only B. Browser console/page-error lists remained empty.
- Final local validation passes `pnpm typecheck`, Web production build, **166/166** tests across 28 files, EventLog/shared/source-sidecar/Node/Bun smokes, and `pnpm desktop:check` including Rust `cargo check`. `git diff --check` passes. Tauri still reports that full Xcode is not installed. Windows Packages run [29482869976](https://github.com/MattSureham/quorum/actions/runs/29482869976) is green on the preceding `a91cfe7` 163-test checkpoint; a fresh Windows run is required for the Echo legacy-config follow-up.
...
- Historical Echo participants are no longer copied verbatim. The client rebuilds adapter config from strict `text` and `script` allowlists, including per-script-step field/type/size bounds, and drops legacy `permissionPolicy` or other unsupported fields. `type`/`intent` accept primitive strings only; coercible arrays or objects are never written back. Empty cleaned steps are removed, and an empty cleaned script is omitted so top-level `text` remains effective. The daemon independently treats a persisted `script: []` as absent.
- The sanitizer-to-network-schema-to-Echo-runtime integration regression covers array enums, `{toString: null}`, unknown-only steps, fallback text, and already-persisted empty scripts. The three focused files pass 18/18 tests; typecheck and Web production build pass. Browser acceptance delays the matching success, injects an unrelated error, and confirms the pending form remains disabled until its own response arrives.
...
- A pure regression starts with `A.png`, captures a lazy current-list reader, removes A, and completes `B.png`; the result is only B. Composer-state has 3 passing tests and typecheck passes. The same sequence passes in Playwright with a real 700 ms delayed FileReader.
