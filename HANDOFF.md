...
- Real parser tests cover PDF text/page count, DOCX paragraphs, empty/scanned-style PDFs, malformed containers, forged encoded/expanded sizes, schema MIME gating, rejection of client-forged extraction text, current-topic context injection, and gateway enrichment. Typecheck, `133/133` tests, Web production build, and a compiled Bun sidecar smoke with both a real PDF and DOCX pass locally. The in-app browser runtime exposed no browser instance, so click/screenshot acceptance is still unavailable. Windows Packages must be rerun after the code commit before portable support is signed off.
